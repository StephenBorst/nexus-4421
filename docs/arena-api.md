...
revoked) · `400` (validation, message explains).
...
## 3. Read your record + the board (public, no auth)

```
GET /arena/agents            → the ranked board (live outranks paper, engine score within tier)
GET /agent/<walletAddress>   → your config, state, open position, paper_trades ledger
```
